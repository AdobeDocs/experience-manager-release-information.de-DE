...
* Beim Öffnen von /lc/ws tritt eine Ausnahme wegen anfälliger Zeichen im HTML-Parameter auf. Hotfix für CQ-4241481
...
* Die Filterung des Tag-Felds funktioniert bei der Eingabe von Text nicht wie erwartet. NPR-22236: Hotfix für CQ-4236655
...
* Benutzerdefinierte Funktionen für den Regeleditor fügen nach dem Funktionsaufruf ein zusätzliches „;“ hinzu. Daher schlagen sie bei der Validierung fehl, obwohl die benutzerdefinierte Funktion „true“ zurückgibt. NPR-22481: Hotfix für CQ-4235499
...
* Das Datumsformat, das in der Übermittlungsanforderung gesendet wird, sollte an dem Muster in der Komponente für die Datumsauswahl ausgerichtet sein. NPR-22384
...
* AEM Forms-Container-Komponente kann nicht bearbeitet werden, wenn auf der AEM Sites-Seite ein falsches Formular konfiguriert wurde. Hotfix für CQ-4237456
...
* Formulare, die nach dem Veröffentlichen der Seite geändert werden, werden nicht erneut veröffentlicht. NPR-23013: Hotfix für CQ-4236566
...
* Es wird verhindert, dass Browser in HTML-Formularen eingegebene Anmeldedaten speichern. NPR-21762: Hotfix für CQ-4206543
...
* Die PDF/UA-ID fehlt beim Speichern als PDF in Designer. NPR-23137
...
* Benutzende können keine Assets mit langen Dateinamen und definiertem Haftungsausschluss herunterladen. NPR-20254
* ProductAssetsUploader belässt die temporären Dateien des Java™-Cache im Java™-Ordner „TEMP“. NPR-20256: Hotfix für CQ-4221801
...
* Ein Metadaten-Fehler der DAM-Asset-API unterbricht die xmp-write-back-APIs. NPR-20607: Hotfix für CQ-4220455
...
* AEM Dynamic Media Classic (früher Scene7) kann Assets nicht vom AEM-Server synchronisieren. Dieses Problem tritt auf, wenn der Zielpfad in der Cloud-Service-Konfiguration auf einen Unterordner im Stammverzeichnis verweist. CQ-4228265
...
* Der eingegebene Workflow-Name sollte im Feld „Benachrichtigung“ verfügbar sein, wenn eine Seite über „Veröffentlichung verwalten“ veröffentlicht wird.  NPR-20046: Hotfix für CQ-4221586
...
* Unerwartetes Tag am Ende des eingefügten Inhalts in einem Inhaltsfragment. NPR-20413: Hotfix für CQ-4224014
...
* Plug-in für Rich-Text funktioniert nicht, weder für die Inline- noch für die Vollbildleiste in der Touch-optimierten Mehrfeld-Benutzeroberfläche. NPR-20973
...
* Nach dem Erstellen einer Communities-Site können Benutzende die Site nicht von der Sitemap aus öffnen, da der Kontextpfad nicht der URL vorangestellt wird. NPR-20730
...
* Bei der Cloud-Service-Konfiguration von Search&amp;Promote wird die Methode zum Abrufen von Formularen ignoriert, sodass sie auf der Autoreninstanz nicht mehr verwendet werden kann. NPR-20447: Hotfix für CQ-4206076
...
* Metadateneigenschaften für Assets des Typs TXT werden in verschiedenen Metadaten-Editoren bei jedem Anzeigen ihrer Eigenschaften angezeigt.  NPR-20239
...
* Die Anmerkungsebene des Seiten-Editors funktioniert in Internet Explorer 11 nicht ordnungsgemäß. NPR-20222: Hotfix für CQ-4222818
* Sitzungsfehler beim Festlegen des Benutzeragenten als Administrator im Replikationsagenten. NPR-20578
* Die Funktion „requestAttributes“ ist jetzt für die anderen „data-sly-resource“-Anweisungen nicht mehr festgelegt.  NPR-20639: Hotfix für 4226206
* Es wurden Probleme mit Curl Head-Anforderungen für OOTB-Assets in AEM behoben. NPR-20781: Hotfix für CQ-4221520

### Projekte {#projects-1}

* Beim Zugriff auf verschiedene Projekte über die Projektkonsole dauert das Laden länger.  NPR-20314
* Bei der Installation von AEM 6.3.0.1 wird der Keystore des Benutzers für `dam-update-service` entfernt. NPR-20018
* Bei einigen benutzerdefinierten Bereitstellungen dauert es länger, bis für Benutzende, die im addTask-Modul eine verantwortliche Person auswählen möchten, die Liste im UserPicker gefüllt wird. NPR-20283: Hotfix für CQ-4224193
...
* Dropdown zeigt den zuletzt gespeicherten Wert an, auch wenn der zugehörige Formulardatenmodell-Service keinen Wert ausgibt. NPR-20710
...
* Der Wert der Switch-Komponente kann nicht auf „true“ oder „false“ festgelegt werden. NPR-21111
...
* Aktivierung der Aktion „In Brand Portal veröffentlichen“ für das standardmäßige Metadatenschemaformular.
* Verhaltensänderung bei der Anzeige von Titeln auf der Bildkarte für ein Bild, dessen Eigenschaft „dc: title“ auf `String [] (multifield)` eingestellt ist.
* Server-seitiges Rendern der Zeit durch eine Foundation-Time-Komponente ersetzt.
...
* Wenn zwei Felder mit derselben Eigenschaft unterschiedlichen Typen von Eigenschaftsfeldern zugeordnet werden, wird ein interner Fehler ausgegeben. NPR-19462: HF für CQ-4216828
...
* Die vollständige Liste der Seiten/Assets kann mit Chrome nicht in der Spaltenansicht geladen werden. NPR-19566: Hotfix für CQ-4214248
...
* Das Targeting-Engine-Feld im Konfigurationsassistenten ist auf ContextHub (AEM) anstatt auf Adobe Target eingestellt. NPR-19320: Hotfix für CQ-4218465
...
* Die Funktionen „Fett“, „Kursiv“ und „Unterstrichen“ funktionieren im Autorendialogfeld nicht für Felder mit Rich-Text-Editor. NPR-19670: NPR-19718: Hotfix für CQ-4219088
...
* Benutzerdefinierte Tag-Bibliotheken, die auf als Skript implementierte Tags verweisen, werden nach der Aktualisierung auf AEM 6.3 NPR-19087 nicht gefunden. 
...
* Die Schaltfläche „Workflow starten“ hat nach Auswahl eines XF-Varianteneditors keine Auswirkungen. NPR-19642: Hotfix für CQ-4207796

### Projekte {#projects-2}

* Projektbearbeitende können keine Assets in den Asset-Ordner des Projekts kopieren und einfügen. NPR-19619: Hotfix für CQ-4215321

### Verwaltung von Web-Inhalten {#web-content-management}

* Auf dem Bildschirm „Rollout“ können Kontrollkästchen für Live Copy-Seiten nicht aktiviert oder deaktiviert werden. NPR-19518
...
* Vorkonfigurierte Eigenschaften und Bildausrichtungseigenschaften werden nicht angezeigt, wenn ein Bild in der klassischen Benutzeroberfläche bearbeitet wird. NPR-19488: Hotfix für CQ-4217914

### Benutzeroberfläche {#user-interface-4}

* Bei Verwendung des Google Chrome-Browsers können Benutzende in der Touch-optimierten Benutzeroberfläche nicht die vollständige Liste der Seiten/Assets mithilfe der Spaltenansicht laden. NPR-19566: Hotfix für CQ-4214248

### Brand Portal {#brand-portal-1}

* Assets können nicht mit Kommentaren und Anmerkungen aus AEM veröffentlicht werden. NPR-19590: Hotfix für CQ-4218386
* Ermöglichen der Veröffentlichung von Tags von AEM in Brand Portal über die Tagadmin-/Tagging-Konsole. NPR-20271: Hotfix für CQ-4223948
...
* Beim Senden an den JEE-Workflow wird der Ausgabeparameter nicht von JEE-Seite an AEM zurückgegeben und der Fehler „Wird ignoriert, weil dies kein primitiver Wert ist“ angezeigt.  NPR-20265
* Adaptive Formulare lassen PDFs nicht als Anhang in Safari zu. NPR-19625
* RestoreGuideState überschreibt die Zuordnung customcontextproperty.  CQ-4222877
* Bei der Konfiguration von Google `reCaptcha` mithilfe eines Cloud-Services erfolgen POST-Aufrufe in Umgebungen, die eine Konfiguration von org.apache.http.proxyconfigurator für externe Verbindungen erfordern, offenbar nicht über PROXY. NPR-20454
* Adaptive Formulare, die auf XSD-Schemata basieren, senden fehlerhafte XML-Werte für numerische Felder bei Installationen mit Gebietsschemata, die über ein anderes Dezimaltrennzeichen als „.“ verfügen, was zu Fehlern führt. NPR-20444
* Proxy-Einstellungen, die für „Apache HTTP Components Proxy Configuration“ festgelegt wurden, werden bei HTTP-Anfragen an Drittanbieter-Server nicht berücksichtigt.  Verbindungs-Timeout-Probleme mit HTTP GET- oder POST-Aufrufen. NPR-20457, NPR-20456, NPR-20455, NPR-20451
...
* Registerkarten mit Anlagen, Notizen und Details zum Workflow funktionieren im Arbeitsbereich nicht, wenn das Formular maximiert/minimiert und als Entwurf gespeichert oder weitergeleitet wird. NPR-20243
* Mehrzeiliges Textfeld (TextArea) behält keine neuen Zeilenzeichen oder Umbrüche im Text bei, nachdem Daten im HTML-Arbeitsbereich übermittelt wurden. NPR-20085

#### Prozessberichterstellung {#process-reporting}

* Prozess-Reporting wird aufgrund der „Null Pointer“-Ausnahme nicht korrekt abgerufen. NPR-19759
...
* Das Dropdown-Metadatenschema-Feld kann nicht auf „obligatorisch“ gesetzt werden, da die obligatorische Feldüberprüfung für Assets fehlt. NPR-17882: FP für CQ-4208373
...
* Beim Hochladen doppelter Assets wird die Option zum Löschen/Beibehalten auf alle Assets angewendet, auch wenn sie nicht im Dialogfeld „Duplikate erkannt“ ausgewählt ist. CQ-4201673
...
* Benutzende mit Nur-Lese-Berechtigungen für die App können keine Vorschau von Inhalten aus dem AEM Mobile-Content-Management anzeigen. NPR-17486: CFP für CQ-4209690
...
* Geänderte lokalisierte Komponenten werden nicht in ihrer ursprünglichen Form wiederhergestellt, wenn die Vererbung in einer LiveCopy erneut angewendet wird. NPR-18172: Hotfix für CQ-4211379
...
* Der Seitenname wird nicht gemäß der Regel für lange Seitentitel generiert. NPR-17633: Hotfix für CQ-4209056
...
* Korrekturen bei Korrespondenzverwaltungs-Textmodulen, Briefvorschauen und programmgesteuertem Starten der Benutzeroberfläche zum Erstellen in Correspondence Management.
...
* Korrekturen an der Benutzerfreundlichkeit für Korrespondenzverwaltung, Dokumentensicherheit und Forms Workflow.
...
* Der zusätzliche Rand über und unter einer Liste mit Aufzählungszeichen ist zwar im Texteditor sichtbar, nicht jedoch in der HTML- und PDF-Ausgabedarstellung. NPR-18126
...
* Der PDF Generator-Service kann Bilder, die größer als 100 MB sind, nicht in PDF-Dokumente konvertieren. CQ-4208628
* Bei Verwendung des PDF Generator-Services mit japanischer Zeichenerkennung wird eine invertierte PDF generiert. NPR-17602
...
>[!NOTE]
>
>Adobe empfiehlt, die Überprüfung für jedes neue Installationspaket auszuführen, bevor Sie versuchen, das Paket zu installieren.  Vor der Überprüfung werden alle Fehler analysiert und protokolliert, die vor der Installation gefunden wurden, und die Benutzer werden proaktiv vor solchen Fehlern gewarnt.
>
>Sie finden die Dokumentation zur Validierungsoption unter [https://experienceleague.adobe.com/de/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions](https://experienceleague.adobe.com/de/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)

* AEM 6.3.3.0 ist eine Voraussetzung für CFP. Detaillierte Anweisungen zum Aktualisieren einer AEM-Installation auf AEM 6.3 finden Sie in der [Upgrade-Dokumentation.](https://experienceleague.adobe.com/de/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
...
>[!NOTE]
>
>Wenn Sie Version 6.3.3.0 oder höher installieren, führen Sie das folgende Verfahren aus, um die Einstellungen für den JBoss®-Anwendungs-Server zu konfigurieren. Wenn Sie 6.3.3.0 auf dem AEM Forms Server installieren, der auf Oracle WebLogic- oder IBM® WebSphere-Programm-Servern ausgeführt wird, ist keine zusätzliche Konfiguration erforderlich. Weitere Informationen finden Sie unter [AEM 6.3.3.0 – Versionshinweise](https://experienceleague.adobe.com/de/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* Seit 6.3.1.2 wurde die Schaltfläche „In Brand Portal veröffentlichen“ für Smart-Sammlungen entfernt.
...
* In der Admin-Suchfunktion gibt es aufgrund zahlreicher Anfragen eine hohe CPU-Auslastung. NPR-24229
...
Der Standardwert ist 1. Überschreiben Sie den Wert durch Überlagern der Datei „constants.js“ (`/libs/cq/ui/widgets/source/constants.js`), indem die Eigenschaft `CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL` auskommentiert wird. Anschließend weisen Sie ihr den erforderlichen Wert zu (die maximale Tiefe unter dem `/jcr:content` der Seite, in dem die Daten der Seiteneigenschaften gespeichert werden).
...
* Rufen Sie das Servlet mithilfe von `/system/console/ configMgr` ab.
...
Durch diesen Prozess kann das Sling GET-Servlet alle erforderlichen Knoten ausgeben.

## Uber Jar {#uber-jar}

Uber Jar für 6.3.3.8 ist im öffentlichen Maven-Repository von Adobe verfügbar.

Um UberJar in einem Maven-Projekt zu verwenden, lesen Sie bitte [Verwendung von UberJar](https://experienceleague.adobe.com/de/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) und nehmen Sie die folgende Abhängigkeit in Ihre Projekt-POM auf:
